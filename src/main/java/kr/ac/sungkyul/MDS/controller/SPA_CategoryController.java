...
public class SPA_CategoryController {
	...
	public String categorylistModify(@PathVariable String domain, HttpSession session, String msgchangeName,
			int msgchangecateNo) {
		// 쇼핑몰 관리자 세션확인
		if (!SPA_mainservice.isUserCheck(domain, session)) {
			return "redirect:/main/loginfrom";
		...
		String url = "http://localhost:8088/Project_MDS/SPA/12maket/category?group_no=" + vo.getCategorylist_group();
	...
	public String insertCategoryV2(@PathVariable String domain,
			// 새로운 2차 카테고리 추가
			@RequestParam(value = "newCategorie") String newCategorie, @RequestParam(value = "group_no") int group_no,
			@RequestParam(value = "count_order") int count_order, HttpSession session) {
		...
		return "redirect:/SPA/" + domain + "/category?group_no=" + group_no;
	}
